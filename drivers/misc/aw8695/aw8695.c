...
#define AW8695_DRIVER_VERSION "v1.4.11"
...
//hch 20190917
static ssize_t aw8695_gun_type_show(struct device *dev,
...
static ssize_t aw8695_gun_type_store(struct device *dev,
...
//hch 20190917
static ssize_t aw8695_bullet_nr_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static ssize_t aw8695_bullet_nr_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
    ...
    mutex_lock(&aw8695->lock);
    aw8695->bullet_nr = val;
    mutex_unlock(&aw8695->lock);
    return count;
}

//hch 20190917
static ssize_t aw8695_gun_mode_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static ssize_t aw8695_gun_mode_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
static int aw8695_haptic_init(struct aw8695 *aw8695)
{
    ...
    aw8695->gun_type = 0xff;
    aw8695->bullet_nr = 0x00;
    aw8695->gun_mode = 0x00;
...
static DEVICE_ATTR(gun_type, S_IWUSR | S_IRUGO, aw8695_gun_type_show,
            aw8695_gun_type_store);	//hch 20190917
static DEVICE_ATTR(bullet_nr, S_IWUSR | S_IRUGO, aw8695_bullet_nr_show,
            aw8695_bullet_nr_store);	//hch 20190917
static DEVICE_ATTR(gun_mode, S_IWUSR | S_IRUGO, aw8695_gun_mode_show,
            aw8695_gun_mode_store);	//hch 20190917
