...
static int td4310_ebbgplus_e7_enable(struct drm_panel *panel)
{
	struct td4310_ebbgplus_e7 *ctx = to_td4310_ebbgplus_e7(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;
...
static int td4310_ebbgplus_e7_disable(struct drm_panel *panel)
{
	struct td4310_ebbgplus_e7 *ctx = to_td4310_ebbgplus_e7(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;
					...
					struct drm_connector *connector)
{
	...
	mode = drm_mode_duplicate(connector->dev, &td4310_ebbgplus_e7_mode);
	if (!mode)
		return -ENOMEM;
...
static const struct drm_panel_funcs td4310_ebbgplus_e7_panel_funcs = {
	...
	.enable = td4310_ebbgplus_e7_enable,
	.unprepare = td4310_ebbgplus_e7_unprepare,
	.disable= td4310_ebbgplus_e7_disable,
