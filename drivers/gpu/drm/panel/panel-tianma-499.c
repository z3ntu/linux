...
static int tianma_499_enable(struct drm_panel *panel)
{
	struct tianma_499 *ctx = to_tianma_499(panel);
	int ret;
...
static int tianma_499_disable(struct drm_panel *panel)
{
	struct tianma_499 *ctx = to_tianma_499(panel);
	int ret;
...
static int tianma_499_get_modes(struct drm_panel *panel)
{
	...
	mode = drm_mode_duplicate(panel->drm, &tianma_499_mode);
	if (!mode)
		return -ENOMEM;
...
static int tianma_499_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx->backlight = tianma_499_create_backlight(dsi);
